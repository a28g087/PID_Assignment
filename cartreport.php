...
if(isset($_POST["customername"]) && ($_POST["customername"]!="")){
	...
	//購物車結束	
	//新增訂單資料
	$sql_query = sprintf("INSERT INTO 
		orders (m_username ,total ,deliverfee ,grandtotal 
			,customername ,customeremail ,customeraddress ,customerphone ,paytype) 
		VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s')"
		, $_SESSION["username"], $cart->total,$cart->deliverfee, $cart->grandtotal
		, $_POST["customername"], $_POST["customeremail"], $_POST["customeraddress"]
		, $_POST["customerphone"], $_POST["paytype"]);
	...
	$sql=sprintf("DELETE FROM cart WHERE username='%s'",$_SESSION["username"]);
	$db_link->query($sql);
